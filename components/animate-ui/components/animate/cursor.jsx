// import * as React from 'react';

// import {
//   CursorProvider as CursorProviderPrimitive,
//   Cursor as CursorPrimitive,
//   CursorFollow as CursorFollowPrimitive,
//   CursorContainer as CursorContainerPrimitive,
// } from '@/components/animate-ui/primitives/animate/cursor';
// // import { cn } from '@/lib/utils';
// import { cn } from '@/lib/utils';

// function CursorProvider({
//   global,
//   ...props
// }) {
//   return (
//     <CursorProviderPrimitive global={global}>
//       <CursorContainerPrimitive {...props} />
//     </CursorProviderPrimitive>
//   );
// }

// function Cursor({
//   className,
//   ...props
// }) {
//   return (
//     <CursorPrimitive asChild {...props}>
//       <svg
//         className={cn('size-6 text-white', className)}
//         xmlns="http://www.w3.org/2000/svg"
//         viewBox="0 0 40 40">
//         <path
//           fill="currentColor"
//           d="M1.8 4.4 7 36.2c.3 1.8 2.6 2.3 3.6.8l3.9-5.7c1.7-2.5 4.5-4.1 7.5-4.3l6.9-.5c1.8-.1 2.5-2.4 1.1-3.5L5 2.5c-1.4-1.1-3.5 0-3.3 1.9Z" />
//       </svg>
//     </CursorPrimitive>
//   );
// }

// function CursorFollow({
//   className,
//   children,
//   sideOffset = 15,
//   alignOffset = 5,
//   ...props
// }) {
//   return (
//     <CursorFollowPrimitive sideOffset={sideOffset} alignOffset={alignOffset} asChild {...props}>
//       <div
//         className={cn('bg-foreground rounded-md text-background px-2 py-2 text-sm', className)}>
//         {children}
//       </div>
//     </CursorFollowPrimitive>
//   );
// }

// export { CursorProvider, Cursor, CursorFollow };

import * as React from 'react';

import {
  CursorProvider as CursorProviderPrimitive,
  Cursor as CursorPrimitive,
  CursorFollow as CursorFollowPrimitive,
  CursorContainer as CursorContainerPrimitive,
} from '@/components/animate-ui/primitives/animate/cursor';
import { cn } from '@/lib/utils';

function CursorProvider({
  global,
  ...props
}) {
  return (
    <CursorProviderPrimitive global={global}>
      <CursorContainerPrimitive {...props} />
    </CursorProviderPrimitive>
  );
}

function Cursor({
  className,
  ...props
}) {
  return (
    <CursorPrimitive asChild {...props}>
      <svg
        // CHANGE 1: Added 'mix-blend-difference'
        // 'text-white' is crucial here. White - Background = Inverted Color.
        className={cn('size-6 text-white mix-blend-difference', className)}
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 40 40">
        <path
          fill="currentColor"
          d="M1.8 4.4 7 36.2c.3 1.8 2.6 2.3 3.6.8l3.9-5.7c1.7-2.5 4.5-4.1 7.5-4.3l6.9-.5c1.8-.1 2.5-2.4 1.1-3.5L5 2.5c-1.4-1.1-3.5 0-3.3 1.9Z" />
      </svg>
    </CursorPrimitive>
  );
}

function CursorFollow({
  className,
  children,
  sideOffset = 15,
  alignOffset = 5,
  ...props
}) {
  return (
    <CursorFollowPrimitive sideOffset={sideOffset} alignOffset={alignOffset} asChild {...props}>
      <div
        // CHANGE 2: Added 'mix-blend-difference'
        // CHANGE 3: Changed colors to 'bg-white text-black'
        // For blending to work, the "ink" must be white. 
        // This ensures: 
        // On Black BG -> Becomes White Box / Black Text
        // On White BG -> Becomes Black Box / White Text
        className={cn(
          'bg-white text-black rounded-md px-2 py-2 text-sm mix-blend-difference', 
          className
        )}>
        {children}
      </div>
    </CursorFollowPrimitive>
  );
}

export { CursorProvider, Cursor, CursorFollow };