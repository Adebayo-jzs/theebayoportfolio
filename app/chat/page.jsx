...
export default function AskAboutBayo() {
  ...
  useEffect(() => {
    // Only auto-focus on desktop screens to prevent mobile keyboard layout jumps
    if (chatOpen && inputRef.current && window.innerWidth >= 768) {
  ...
  return (
    <main className="min-h-dvh h-dvh max-h-dvh bg-background flex flex-col items-center justify-center relative overflow-hidden">
        ...
        {menuOpen && (
          <>
            ...
            <motion.nav
              ...
              className="fixed top-16 right-4 sm:top-20 sm:right-6 md:top-22 md:right-8 z-50 w-[calc(100vw-2rem)] max-w-xs bg-card/95 backdrop-blur-xl border border-border/70 rounded-2xl shadow-2xl p-2.5 space-y-1.5"
            >
          ...
          {!chatOpen && (
            ...
            >
              ...
              <button
                ...
                className="group relative overflow-hidden border border-foreground/20 bg-foreground text-background px-8 py-3 text-sm font-semibold transition-all duration-300 hover:scale-[1.02] hover:shadow-lg hover:shadow-foreground/5 active:scale-[0.98] rounded-lg cursor-pointer"
              >
        ...
        <AnimatePresence mode="wait">
          {chatOpen && (
            <motion.div
              ...
              className="flex-1 flex flex-col h-full max-h-dvh overflow-hidden"
            >
              {/* Header */}
              <div className="shrink-0 z-30 bg-background/95 backdrop-blur-md px-5 pt-4 pb-3 border-b border-border/40">
              ...
              {/* Messages area */}
              <div className="flex-1 overflow-y-auto px-5 py-5 space-y-6 hide-scrollbar overscroll-contain">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {suggestedQuestions.map((q) => (
                        <button
                          ...
                          className="text-xs text-muted-foreground border border-border/60 rounded-full px-4 py-2 hover:bg-accent hover:text-foreground transition-colors duration-200 text-left"
                        >
                          {q}
                        </button>
                      ))}
